...
let calcul_exprp ep env mem =
  ...
  | ASTAdr lval -> (calcul_lval lval env mem),mem
...
let rec calcul_def expr env mem =
  ...
  (*3*)
  | ASTFunb (id, t, args, bk) -> 
      (Couple(id, InP (bk, get_argsp args,env))::env,mem)
  | ASTFunRecb ( id, t, args, block) -> 
      (Couple(id, InPR (InP (bk, get_argsp args, env))) :: env, mem)
...
let rec calcul_cmds cmds env mem flux =
  ...
  | ASTReturn (ret) -> calcul_ret ret env mem output
  (*END*)
  | ASTStat stat -> calcul_instr stat env mem flux
(*| _ -> failwith "erreur calcul_cmds"*)

(*3*)  
and calcul_ret ret env mem output =
  match ret with
    ASTRet (e) -> let _,mem1 = calcul_expression e env mem in mem1,[]
